...
export default function Sales() {
  ...
  const salesByPaymentMethod = {
    CASH: filteredSales.filter(sale => sale.paymentMethod === 'CASH').reduce((total, sale) => total + sale.total, 0),
    CARD: filteredSales.filter(sale => sale.paymentMethod === 'CARD').reduce((total, sale) => total + sale.total, 0),
    TRANSFER: filteredSales.filter(sale => sale.paymentMethod === 'TRANSFER').reduce((total, sale) => total + sale.total, 0)
  };
  ...
  const getPaymentMethodInfo = (method: string) => {
    switch(method) {
      case 'CASH':
        return { text: 'Μετρητά', icon: <BanknotesIcon className="h-5 w-5 text-gray-400" /> };
      case 'CARD':
        return { text: 'Κάρτα', icon: <CreditCardIcon className="h-5 w-5 text-gray-400" /> };
      case 'TRANSFER':
        return { text: 'Τραπεζική Μεταφορά', icon: <ArrowPathIcon className="h-5 w-5 text-gray-400" /> };
      default:
        return { text: method, icon: <CurrencyEuroIcon className="h-5 w-5 text-gray-400" /> };
    }
  };
  ...
  return (
    <Layout>
      {/* Header section */}
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Διαχείριση Πωλήσεων</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Συνολικές Πωλήσεις</p>
              <p className="text-xl font-bold mt-1 text-gray-900 dark:text-white">{totalSales.toFixed(2)} €</p>
            </div>
          ...
          <div className="mt-2 flex items-center text-xs">
            ...
            <span className="ml-1 text-gray-500 dark:text-gray-400">από τον προηγούμενο μήνα</span>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Μετρητά</p>
              <p className="text-xl font-bold mt-1 text-gray-900 dark:text-white">{salesByPaymentMethod.CASH.toFixed(2)} €</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          <div className="flex justify-between items-start">
            <div>
              ...
              <p className="text-xl font-bold mt-1 text-gray-900 dark:text-white">{salesByPaymentMethod.CARD.toFixed(2)} €</p>
            </div>
        ...
        <div className="bg-white dark:bg-gray-800 p-4 rounded-lg shadow">
          <div className="flex justify-between items-start">
            <div>
              <p className="text-sm text-gray-500 dark:text-gray-400">Τραπεζική Μεταφορά</p>
              <p className="text-xl font-bold mt-1 text-gray-900 dark:text-white">{salesByPaymentMethod.TRANSFER.toFixed(2)} €</p>
            </div>
      ...
      <div className="flex flex-col md:flex-row gap-4 mb-6">
        ...
        <div className="w-full md:w-1/4">
          <select
            ...
            name="payment-filter"
            className="mt-1 block w-full pl-3 pr-10 py-2 text-base border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 sm:text-sm rounded-md dark:bg-gray-700 dark:border-gray-600 dark:text-white"
            onChange={(e) => setPaymentMethodFilter(e.target.value as 'all' | 'CASH' | 'CARD' | 'TRANSFER')}
          ...
          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
            <thead className="bg-gray-50 dark:bg-gray-700">
              <tr>
                ...
                <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider">Τρόπος Πληρωμής</th>
                  ...
                  <React.Fragment key={sale.id}>
                    <tr className="hover:bg-gray-50 dark:hover:bg-gray-700">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        <div className="text-xs text-gray-500 dark:text-gray-400">ID: {sale.customerId}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="flex items-center text-sm text-gray-900 dark:text-white">
                          ...
                          <span className="ml-1">{getPaymentMethodInfo(sale.paymentMethod).text}</span>
                        </div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-medium text-gray-900 dark:text-white">{sale.total.toFixed(2)} €</div>
                        <div className="text-xs text-gray-500 dark:text-gray-400">{sale.items.length} προϊόντα</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <div className="flex justify-end space-x-2">
                          ...
                          >
                            ...
                            <span className="sr-only md:not-sr-only md:inline-block">Τιμολόγιο</span>
                          </button>
                        </div>
                      </td>
                    </tr>
                    {showDetails === sale.id && (
                      <tr className="bg-gray-50 dark:bg-gray-700">
                        <td colSpan={6} className="px-6 py-4">
                          <div className="text-sm font-medium text-gray-900 dark:text-white mb-2">Στοιχεία Πώλησης</div>
                          <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                            <thead>
                              <tr>
                                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Προϊόν/Υπηρεσία</th>
                                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Τύπος</th>
                                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Τιμή</th>
                                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Ποσότητα</th>
                                <th scope="col" className="px-4 py-2 text-left text-xs font-medium text-gray-500 dark:text-gray-400">Σύνολο</th>
                              </tr>
                            </thead>
                                ...
                                <tr key={index}>
                                  ...
                                  <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500 dark:text-gray-400">
                                    <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${
                                      ...
                                      {item.type === 'SERVICE' ? 'Υπηρεσία' : 'Προϊόν'}
                                    </span>
                                  </td>
                                  ...
                                  <td className="px-4 py-2 whitespace-nowrap text-sm font-medium text-gray-900 dark:text-white">{(item.price * item.quantity).toFixed(2)} €</td>
                                </tr>
                              ))}
                            </tbody>
                            <tfoot>
                              <tr>
                                <td colSpan={4} className="px-4 py-2 text-sm font-medium text-right text-gray-900 dark:text-white">Συνολικό Ποσό:</td>
                                <td className="px-4 py-2 text-sm font-bold text-gray-900 dark:text-white">{sale.total.toFixed(2)} €</td>
                              </tr>
                            </tfoot>
                          </table>
                        </td>
        ...
        <div className="fixed inset-0 z-10 overflow-y-auto bg-gray-500 bg-opacity-75 flex items-center justify-center">
          <div className="bg-white dark:bg-gray-800 rounded-lg max-w-lg w-full p-6">
            <div className="flex justify-between items-start mb-4">
              <h3 className="text-lg font-medium text-gray-900 dark:text-white">Προσθήκη Νέας Πώλησης</h3>
              <button
                type="button"
            ...
            <form className="space-y-4">
                ...
                <div className="bg-gray-50 dark:bg-gray-700 p-3 rounded-md space-y-3">
                  ...
                  <div className="grid grid-cols-12 gap-2">
                    <div className="col-span-5">
                      <select
                        className="block w-full pl-3 pr-10 py-2 text-sm border-gray-300 focus:outline-none focus:ring-primary-500 focus:border-primary-500 rounded-md dark:bg-gray-600 dark:border-gray-500 dark:text-white"
                      >
              ...
              <div className="flex justify-between mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
                <div className="text-sm font-medium text-gray-700 dark:text-gray-300">Συνολικό Ποσό:</div>
